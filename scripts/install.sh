...
#
# V2 features:
# - Installs to /home/<user>/growmate/ (runs as non-root user)
...
# ── Detect install source directory ──────────────────────────────────────────
...
if [ -z "$TARGET_USER" ]; then
    if [ -n "$SUDO_USER" ]; then
        ...
        TARGET_USER=$(logname 2>/dev/null || echo "grow")
    else
        TARGET_USER="grow"
    fi
fi
...
INSTALL_DIR="${TARGET_HOME}/growmate"
...
enable_i2c() {
    ...
    if [ -e /dev/i2c-1 ]; then
        log_success "I2C already enabled (found /dev/i2c-1)"
        return
    fi
    ...
    if [ -e /dev/i2c-1 ]; then
        ...
        log_warning "I2C config applied but /dev/i2c-1 not found (reboot may be required)"
    fi
}
...
# See ${INSTALL_DIR}/config/config.yaml.example for full documentation
version: 9
